...
import { Search, Upload, Download, Trash2, FileSpreadsheet, FileText, UserPlus, Users, AlertCircle } from 'lucide-react';
...
const PlacedStudents = () => {
  ...
  const fetchPlacedStudents = async (search = '') => {
    try {
      ...
      setError(error.response?.data?.message || 'Failed to fetch placed students. Please try again.');
    ...
    try {
      ...
      const students = jsonData.map(row => ({
        email: row.email || row.Email || row.EMAIL || 
              `${row.id || row.Id || row.ID || row.studentId || row.StudentId || row.StudentID || row.STUDENT_ID}@charusat.edu.in`
      }));

      const response = await axiosInstance.post('/admin/placed-students/bulk', { students });
      toast.success(`Successfully updated ${response.data.updatedCount} students`);
      setFile(null);
  ...
  const handleSingleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axiosInstance.post('/admin/placed-students/single', payload);
      toast.success(response.data.message || 'Student successfully marked as placed');
      setStudentInput('');
      fetchPlacedStudents(searchQuery);
    } catch (error) {
  ...
  const handleRemoveStudent = async (studentId, studentName) => {
    if (!window.confirm(`Are you sure you want to remove ${studentName} from the placed students list?`)) {
      return;
    }

    try {
      setLoading(true);
      const response = await axiosInstance.post('/admin/placed-students/remove', { studentId });
      toast.success(response.data.message || 'Student removed from placed list');
      fetchPlacedStudents(searchQuery);
    } catch (error) {
  ...
  const exportToExcel = () => {
    try {
      const exportData = placedStudents.map(student => ({
        ...
        'Name': student.name,
        'Email': student.email,
        'Placed Date': student.placedDate ? new Date(student.placedDate).toLocaleDateString() : 'N/A'
      }));
  ...
  const exportToPDF = () => {
    try {
      ...
      const tableRows = placedStudents.map(student => [
        ...
        student.name,
        student.email,
        student.placedDate ? new Date(student.placedDate).toLocaleDateString() : 'N/A'
      ]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      <div className="container mx-auto px-4 py-8">
        <div className="max-w-7xl my-14 mx-auto space-y-8">
          {/* Header */}
          <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-indigo-100 mt-1">Manage and track placed students</p>
                  </div>
                </div>
            ...
            <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <form onSubmit={handleSingleSubmit} className="space-y-4">
                  ...
                  <div className="flex gap-4">
                    <input
                      ...
                      placeholder={inputType === 'email' ? 'Enter student email' : 'Enter student ID'}
              ...
              <div className="bg-white rounded-xl border p-6 space-y-4">
                ...
                <p className='text-sm text-red-900'>Note: Excel file should contain either email or id column for students</p>
              </div>
            </div>
              ...
              <div className="bg-gradient-to-r from-indigo-50 to-blue-50 rounded-xl p-6 mb-6 border border-indigo-100/50">
                <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h2 className="text-xl font-semibold text-gray-900">Placed Students List</h2>
                      <p className="text-sm text-gray-600 mt-0.5">View and manage placed students</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    <div className="flex items-center gap-2 px-4 py-2 bg-white rounded-lg border border-indigo-100">
                      ...
                      <span className="text-sm font-semibold text-indigo-600">{totalStudents}</span>
                    </div>
                  </div>
                </div>
              </div>
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="bg-gradient-to-r from-gray-50 to-gray-100">
                        ...
                        <th className="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Placed Date</th>
                        ...
                        <tr>
                          <td colSpan="5" className="px-6 py-8 text-center">
                            <div className="flex flex-col items-center justify-center text-gray-500">
                              ...
                              <p className="text-gray-600 font-medium">No placed students found</p>
                              <p className="text-sm text-gray-500 mt-1">Add students using the forms above</p>
                            </div>
                          </td>
                        </tr>
                      ) : (
                  ...
                  <div className="px-6 py-4 bg-gray-50 border-t border-gray-200">
                    <div className="flex items-center justify-between">
                      <div className="text-sm text-gray-700">
                        ...
                        <span className="font-medium">{Math.min((page + 1) * rowsPerPage, totalStudents)}</span> of{" "}
